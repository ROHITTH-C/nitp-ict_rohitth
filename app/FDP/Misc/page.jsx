"use client";
import React from "react";
import GDriveTable from "../../components/FDP/MiscTable";

const courses = [
    {
      name: "Technology Enabled Teaching Learning and Process for Institute",
      gdrive: "https://drive.google.com/file/d/14or3JBOPJl1lDtgRx-k1SqKfpcfP0zxj/view"
    },
    {
      name: "Text Analytics with Deep Learning",
      gdrive: "https://drive.google.com/file/d/1ZuhBJW6yR_jHepFESPm6hp0OFNT98Diw/view"
    },
    {
      name: "Quantum Computing (IBM)",
      gdrive: "https://drive.google.com/file/d/1JA5KM2dYAUt9FYHaJBCRn6Jx2IHFsF61/view"
    },
    {
      name: "About E&ICT Academy",
      gdrive: "https://drive.google.com/file/d/13O1Kj8M53jGVvHoHU2XIfS2z6p0ykTte/view"
    },
    {
      name: "IoT Applications with Sensors, Embedded Systems, and Data Analytics",
      gdrive: "https://drive.google.com/file/d/1x9l_wLChvmxoq3afTtumRzV3GhpEHNo_/view"
    },
    {
      name: "AI and Deep Learning",
      gdrive: "https://drive.google.com/file/d/1USJ2c4AfJE9XhkytU1jqXaxF08a6UijP/view"
    },
    {
      name: "Generative AI and Applications",
      gdrive: "https://drive.google.com/file/d/1LuL3MAANpGT0QFDxeDWE1LrSoySCTKEG/view"
    },
    {
      name: "Advanced Optimization Techniques with MATLAB",
      gdrive: "https://drive.google.com/file/d/1z2ua6HVdtzYy6Hz_z3ISLKt__0F6gppA/view"
    },
    {
      name: "Data Science for All",
      gdrive: "https://drive.google.com/file/d/1FpwGcxu-9zTjqYTOhgH9rRKcqes58u2t/view"
    },
    {
      name: "V2X and UAV Communications in 6G",
      gdrive: "https://drive.google.com/file/d/1OCaUB2avo712u-kftBuM7MnOIwWrtSxb/view"
    },
    {
      name: "Nanotechnology for VLSI",
      gdrive: "https://drive.google.com/file/d/17LJELbWxx40uYm91Y5FD0yLdobXzJTNm/view"
    },
    {
      name: "Biometric Security in the Generative AI Era",
      gdrive: "https://drive.google.com/file/d/1-BPZ4MpYPDcR0D4oLpXZQksbV0StybW2/view"
    },
    {
      name: "Deep Learning & Optimization for Healthcare",
      gdrive: "https://drive.google.com/file/d/1vd5p9TlEgtq5JKptQErRXfyzHG-ZPXUN/view"
    },
    {
      name: "Application of Digital Technologies in Manufacturing Environment",
      gdrive: "https://drive.google.com/file/d/1_Ln9je38tKrWpYrsgb4ZcWIlq5wicbW6/view"
    },
    {
      name: "Semiconductor Fabrication and Packaging Technology",
      gdrive: "https://drive.google.com/file/d/1ptTSYuupJ8R3c2yICVoMaKSBjAV5I35H/view"
    },
    {
      name: "Electric Vehicles and Energy Storage Systems: Recent Trends and Future Prospects",
      gdrive: "https://drive.google.com/file/d/1sk3G6uSHBkfnkqyN1Xf2KieX0dTfloq6/view"
    },
    {
      name: "Current Challenges in Cyber Security with Blockchain and IDS in IoT Domain",
      gdrive: "https://drive.google.com/file/d/1LfE3C0nTAjEtyhVEQ88vXWamY5gLPCCB/view"
    },
    {
      name: "Advances in Millimeter Wave and Terahertz Technologies for 6G and Future Wireless Networks",
      gdrive: "https://drive.google.com/file/d/1XsO21xBqLx3pEqrCZBIsvZtGAcnIu87V/view"
    },
    {
      name: "Advances in Artificial Intelligence for Image Processing and Medical Applications",
      gdrive: "https://drive.google.com/file/d/1BgFq9T3t1qEQ9EuZYnIU9ZR_H9Ze5PcC/view"
    },
    {
      name: "Application of Digital Technologies in Manufacturing Environment",
      gdrive: "https://drive.google.com/file/d/15oH-c-h4cC5DSUCHP6AZKmHG3BloE1kk/view"
    },
    {
      name: "5G Wireless System Design: From the Standards Perspective",
      gdrive: "https://drive.google.com/file/d/1kDVRzp47VXL6Bnb94f7Oot3YfdKaeTcr/view"
    },
    {
      name: "Applications of Industry 4.0 (I4.0) in Circular Supply Chain",
      gdrive: "https://drive.google.com/file/d/1v4PIXopCw9g6aZCvI2W5sn-ZOV3WyDgU/view"
    }
  ];
  

const FDPPage = () => {
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <GDriveTable courses={courses} />
    </div>
  );
};

export default FDPPage;
